...
<#
.SYNOPSIS
Invokes a Microsoft Graph request with a stored or supplied auth context.
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
...
$parameters = @{
    ...
    AuthContext = Resolve-McpContext -ContextId $ContextId -AuthContext $AuthContext -Purpose 'Graph'
}

if ($PSBoundParameters.ContainsKey('Body')) { $parameters.Body = $Body }
if ($Paginate) { $parameters.Paginate = $true }

$invocation = Invoke-MicrosoftCloudApiSkillScript -RelativePath 'skills/graph/Invoke-GraphRequest.ps1' -Parameters $parameters
