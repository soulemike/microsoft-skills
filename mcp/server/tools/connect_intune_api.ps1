...
<#
.SYNOPSIS
Creates an Intune-compatible Microsoft Graph authentication context and stores it for later MCP tool calls.
#>
[CmdletBinding()]
param(
    [Parameter()]
...
if ($PSBoundParameters.ContainsKey('ContextId') -or $PSBoundParameters.ContainsKey('AuthContext')) {
    $parameters.AuthContext = Resolve-McpContext -ContextId $ContextId -AuthContext $AuthContext -Purpose 'Intune'
}

$invocation = Invoke-MicrosoftCloudApiSkillScript -RelativePath 'skills/graph/Connect-GraphApi.ps1' -Parameters $parameters
$record = Save-McpContextRecord -ContextType 'intune' -Payload @{ AuthContext = $invocation.Result }
